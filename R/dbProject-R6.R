...
#'
#' @description
#' This R6 class provides methods for disconnecting and reconnecting to a DuckDB
#' database connection. It also provides methods to pin lazy database tables to
#' the board.
#' 
#' @details
#' A dbProject is a [pins::board_folder()] that contains a pinned connection object
#' and other pinned objects. The connection object is stored in a pin named
#' "cachedConnection" and is used to reconnect to the database when necessary.
#' 
#'
#' @export
dbProject <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @param drv A DBI driver object, e.g. [duckdb::duckdb()].
    ...
    initialize = function(drv, path, ...) {
      ...
      tryCatch({
        private$conn <- connections::connection_open(drv = duckdb::duckdb(), ...)
    ...
    #' @description
    #' Restore all pins from the board manifest, handling the cached connection separately.
    restore = function() {
      ...
      for (pin_name in names(manifest)) {
        if (pin_name == "cachedConnection") {
          ...
          # Read the pin and assign it to global environment
          pin_value <- self$pin_read(pin_name)
          assign(pin_name, pin_value, envir = .GlobalEnv)
        }
      }
      invisible(self)
    },
    ...
    print = function(...) {
      ...
      if (dir.exists(private$path)) {
        ...
        cli::cli_alert_danger("Board Path: {.path {private$path}} (directory not found)")
      }
      ...
      if (!is.null(private$conn)) {
        ...
        if (!file.exists(dbdir) && dbdir != ":memory:") {
          ...
          tryCatch({
            ...
            views <- dplyr::filter(table_types, grepl("VIEW", table_type, ignore.case = TRUE))
            temp_tables <- dplyr::filter(table_types, grepl("TEMPORARY", table_type, ignore.case = TRUE))
            ...
            cli::cli_alert_warning("Unable to retrieve table information: {e$message}")
          })
        } else {
